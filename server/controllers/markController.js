  ...
  if (!req.body || !req.body.mark || !req.body.mark.mark) {
    ...
    newMark.save((err, saved) => {
      if (err) {
        ...
        res.json({ mark: saved });
        logger.info(`markController.addMark ${newMark.mark}`);
      }
    });
  }
}
...
export function updateMark(req, res) {
  if (!req.body || !req.body.mark) {
    let error = { status: "error", message: "! markController.updateMark failed! - no body or mark" };
    if (!req.body )
      error.message += "... no req.body!";
    if (req.body && !req.body.mark )
      error.message += "... no req.body.mark!";
